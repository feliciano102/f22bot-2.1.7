...
async function sudoCommand(sock, chatId, message) {
    ...
    if (sub === 'add') {
        ...
        await sock.sendMessage(chatId, { text: ok ? `✅ Adicionado sudo: ${targetJid}` : '❌ Falha ao adicionar sudo' },{quoted :message});
        return;
    }
    ...
    if (sub === 'del' || sub === 'remove') {
        ...
        if (targetJid === ownerJid) {
            await sock.sendMessage(chatId, { text: 'O proprietário não pode ser removido.' },{quoted :message});
            return;
        }
        const ok = await removeSudo(targetJid);
        await sock.sendMessage(chatId, { text: ok ? `✅ Removido sudo: ${targetJid}` : '❌ Falha ao remover sudo' },{quoted :message});
        return;
    }
}
